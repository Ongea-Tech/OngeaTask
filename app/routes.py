...
def show_task(task_id):
    task = Task.query.filter_by(user_id=current_user.id, id=task_id).first_or_404()
...
def create_task():
    if request.method == 'POST':
        ...
        new_task = Task(title=name, description=description, user_id=current_user.id, completed=False)
...
def mark_completed_single(task_id):
    try:
        task = db.session.query(Task).with_for_update().filter_by(user_id=current_user.id, id=task_id).first()
...
def history_action():
    ...
    for task_id in task_ids:
        task = Task.query.filter_by(user_id=current_user.id, id=int(task_id)).first()
        if not task:
            continue
...
def delete_selected():
    ...
    if ids:
        ...
        Subtask.query.join(Task).filter(Subtask.id.in_(ids), Task.user_id == current_user.id).delete(synchronize_session=False)
        db.session.commit()
