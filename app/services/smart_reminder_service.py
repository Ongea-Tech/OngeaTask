...
def get_task_nudge_message(task, user_id):
    ...
    if task.priority == "High":
        return f"'{task.title}' is marked as high priority. Consider working on it before lower-priority tasks."
    if readable_hour:
...
def should_nudge_task(task):
    ...
    if task.last_nudged_at:
        hours_since_last_nudge = (now - 
        task.last_nudged_at).total_seconds() / 3600
...
def generate_smart_nudges_for_user(user_id):
    ...
    for task, score in tasks_with_scores[:3]:
        if should_nudge_task(task):
            ...
            task_id=task.id,
            message=message,
            nudge_type="smart_reminder"
            )
            task.last_nudged_at = datetime.utcnow()
