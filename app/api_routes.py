...
def get_active_tasks():
    tasks = Task.query.filter_by(user_id=current_user.id, completed=False, deleted=False).all() 
    result = []
    for task in tasks:
...
def delete_subtask(subtask_id):
    subtask = Subtask.query.filter_by(user_id=current_user.id, id=subtask_id).first_or_404()
...
def mark_completed():
    ...
    for subtask_id in completed_ids:
        subtask = Subtask.query.filter_by(user_id=current_user.id, id=subtask_id).first()
        if subtask:
